...
export default function Admin({ session, profile }) {
  ...
  const [newM, setNewM]         = useState({
    ...
    stance:'Regular',level:'Beginner',favourite_trick:'',idol_skater:'',bio:'',role:'Member'
  })
  ...
  async function load() {
    try {
      const [p,m,b,g,ps,t,r] = await Promise.all([
        getPendingUsers(), getAllMembers(), getAllBookings(),
        getGallery(), getPosts(), getTimetable(), getAllReviews()
      ])
      setPending(p||[]); setMembers(m||[]); setBookings(b||[])
      setGallery(g||[]); setPosts(ps||[]); setTt(t||[]); setReviews(r||[])
  ...
  return (
    <>
      ...
      {toast&&(
        <div style={{position:'fixed',bottom:'1.5rem',right:'1.5rem',background:toastType==='err'?'rgba(30,10,10,0.98)':'rgba(20,30,20,0.98)',border:`1px solid ${toastType==='err'?'#e06060':'#6dc86d'}`,borderLeft:`4px solid ${toastType==='err'?'#e06060':'#6dc86d'}`,padding:'0.85rem 1.2rem',fontFamily:"'Space Mono',monospace",fontSize:'0.72rem',zIndex:9999,maxWidth:340,lineHeight:1.5,color:toastType==='err'?'#e06060':'#6dc86d'}}>
          {toast}
        </div>
      )}
      ...
      <div style={{paddingTop:'3.3rem',minHeight:'100vh'}}>
        ...
        <div style={A.layout}>
          ...
          <div style={A.side}>
            {NAV.map(n=>(
              <button key={n.id} onClick={()=>setSec(n.id)} style={{...A.ni,...(sec===n.id?A.niOn:{})}}>
                {n.label}
          ...
          <div style={A.main}>
                ...
                <form onSubmit={doAddMember} style={A.formBox}>
                  ...
                  <div style={A.frow}>
                    <Fg label="Date of Birth">
                      ...
                      {newM.date_of_birth&&<div style={A.ageD}>Age: {calcAge(newM.date_of_birth)} years</div>}
                    </Fg>
                  ...
                  {newM.category==='Student'&&(
                    <>
                      <div style={A.sHead}>School Information</div>
                      <div style={A.frow}>
                        <Fg label="School Name"><input className="input" value={newM.school} onChange={e=>setNewM({...newM,school:e.target.value})} placeholder="Name of school"/></Fg>
                        <Fg label="Grade / Class"><input className="input" value={newM.grade} onChange={e=>setNewM({...newM,grade:e.target.value})} placeholder="e.g. Form 3, Grade 8"/></Fg>
                      </div>
                    </>
                  )}
                  ...
                  <div style={A.frow}>
                    <Fg label="Contact Name"><input className="input" value={newM.emergency_name} onChange={e=>setNewM({...newM,emergency_name:e.target.value})} placeholder="e.g. Mama Fatuma"/></Fg>
                    <Fg label="Relationship"><select className="input" value={newM.emergency_relationship} onChange={e=>setNewM({...newM,emergency_relationship:e.target.value})}>{RELS.map(r=><option key={r}>{r}</option>)}</select></Fg>
                  </div>
                  <Fg label="Emergency Phone"><input className="input" type="tel" value={newM.emergency_phone} onChange={e=>setNewM({...newM,emergency_phone:e.target.value})} placeholder="+255 ..."/></Fg>

                  <div style={A.sHead}>Skating Profile</div>
                  <div style={A.frow}>
                    <Fg label="Stance"><select className="input" value={newM.stance} onChange={e=>setNewM({...newM,stance:e.target.value})}>{STANCES.map(s=><option key={s}>{s}</option>)}</select></Fg>
                    <Fg label="Level"><select className="input" value={newM.level} onChange={e=>setNewM({...newM,level:e.target.value})}>{LEVELS.map(l=><option key={l}>{l}</option>)}</select></Fg>
                  </div>
                  <div style={A.frow}>
            ...
            {sec==='gallery'&&(
              <div>
                ...
                {gallery.length>0&&(
                  <div style={{display:'grid',gridTemplateColumns:'repeat(auto-fill,minmax(150px,1fr))',gap:'0.8rem',marginTop:'1.5rem'}}>
                    {gallery.map(item=>(
                      <div key={item.id}>
            ...
            {sec==='posts'&&(
              <div>
                ...
                <form onSubmit={doPublishPost} style={A.formBox}>
                  ...
                  <Fg label="Content *"><textarea className="input" rows={5} required value={postForm.content} onChange={e=>setPostForm({...postForm,content:e.target.value})} placeholder="Write your update..." style={{resize:'vertical'}}/></Fg>
                  <Fg label="Attach Photo or Video (optional)">
                    <div style={{display:'flex',gap:'0.7rem',alignItems:'center',flexWrap:'wrap',marginBottom:'0.6rem'}}>
                      <button type="button" style={A.btn} onClick={()=>document.getElementById('postMediaIn').click()}>📎 Attach Media</button>
            ...
            {sec==='schedule'&&(
              <div>
                ...
                <button className="btn-primary" style={{marginBottom:'1rem'}} onClick={()=>setShowAddTT(!showAddTT)}>{showAddTT?'Cancel':'+ Add New Session'}</button>
                {showAddTT&&(
...
function EditMemberModal({ m, onSave, onClose, onPhoto, calcAge }) {
  const [f, setF] = useState({
    ...
    emergency_relationship: m.emergency_relationship||'Parent',
  ...
  return (
    <div style={{position:'fixed',inset:0,background:'rgba(0,0,0,0.92)',zIndex:500,display:'flex',alignItems:'center',justifyContent:'center',padding:'1rem'}}>
      <div style={{background:'#161616',border:'1px solid rgba(200,162,50,0.35)',padding:'2rem',width:620,maxWidth:'96vw',maxHeight:'93vh',overflowY:'auto',position:'relative'}}>
        ...
        <div style={{display:'grid',gridTemplateColumns:'140px 1fr',gap:'1.5rem',marginBottom:'1.3rem'}}>
          <div>
            <div style={{width:135,height:135,background:'#5c1212',display:'flex',alignItems:'center',justifyContent:'center',fontFamily:"'Bebas Neue',sans-serif",fontSize:'2.8rem',color:'#c8a232',overflow:'hidden',position:'relative',marginBottom:'0.6rem'}}>
              {photoPrev?<img src={photoPrev} alt={m.full_name} style={{position:'absolute',inset:0,width:'100%',height:'100%',objectFit:'cover'}}/>:init}
            </div>
            <button style={{...A.btn,width:135,textAlign:'center',display:'block'}} onClick={()=>document.getElementById('pmPhIn').click()}>📷 Upload Photo</button>
          ...
          <div>
            ...
            <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:'0.6rem',marginBottom:'0.85rem'}}>
              ...
              <div><label className="label">Age (auto)</label><div style={{...A.gold,paddingTop:'0.65rem',fontSize:'1rem'}}>{f.date_of_birth?calcAge(f.date_of_birth)+'y':'—'}</div></div>
            </div>
        ...
        {f.category==='Student'&&(
          <>
            ...
            <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:'0.75rem',marginBottom:'0.85rem'}}>
              <Fg label="School Name"><input className="input" value={f.school} onChange={e=>setF({...f,school:e.target.value})} placeholder="Name of school"/></Fg>
              <Fg label="Grade / Class"><input className="input" value={f.grade} onChange={e=>setF({...f,grade:e.target.value})} placeholder="e.g. Form 3"/></Fg>
            </div>
          </>
        )}
        ...
        <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:'0.75rem',marginBottom:'0.85rem'}}>
          <Fg label="Relationship"><select className="input" value={f.emergency_relationship} onChange={e=>setF({...f,emergency_relationship:e.target.value})}>{RELS.map(r=><option key={r}>{r}</option>)}</select></Fg>
        ...
        <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:'0.75rem',marginBottom:'0.85rem'}}>
          ...
          <Fg label="Level"><select className="input" value={f.level} onChange={e=>setF({...f,level:e.target.value})}>{LEVELS.map(l=><option key={l}>{l}</option>)}</select></Fg>
        </div>
        <Fg label="Favourite Trick"><input className="input" value={f.favourite_trick} onChange={e=>setF({...f,favourite_trick:e.target.value})} placeholder="e.g. Kickflip, Ollie..."/></Fg>
        ...
        <Fg label="Bio"><textarea className="input" rows={3} value={f.bio} onChange={e=>setF({...f,bio:e.target.value})} placeholder="Short bio..." style={{resize:'vertical'}}/></Fg>
...
const A = {
  ...
  sHead:    {fontFamily:"'Bebas Neue',sans-serif",fontSize:'1.1rem',color:'#c8a232',margin:'1.2rem 0 0.7rem',borderBottom:'1px solid rgba(200,162,50,0.2)',paddingBottom:'0.3rem'},
  ...
  ageD:     {fontFamily:"'Space Mono',monospace",fontSize:'0.75rem',color:'#c8a232',marginTop:'0.3rem'},
  ...
  reviewCard:{padding:'1rem',border:'1px solid rgba(255,255,255,0.08)',marginBottom:'0.8rem',background:'rgba(255,255,255,0.02)'},
