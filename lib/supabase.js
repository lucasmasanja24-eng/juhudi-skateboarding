...
// ── PHOTO UPLOAD — CLEAN PERMANENT FIX ───────────────────────
export async function uploadProfilePhoto(userId, file) {
  // Always use the same fixed filename per user: profiles/USERID.ext
  // This means uploading a new photo always replaces the old one cleanly
  const ext = file.name.split('.').pop().toLowerCase().replace('jpeg','jpg')
  const path = `${userId}.${ext}`

  // Upload with upsert:true so it overwrites existing file
  const { error: uploadError } = await supabase.storage
    .from('profiles')
    .upload(path, file, {
      upsert: true,
      cacheControl: '1', // 1 second cache — effectively no cache
    })
  ...
  // Append a version timestamp so browser always loads fresh
  // This is the key fix — every upload gets a unique URL
  const permanentUrl = `${data.publicUrl}?v=${Date.now()}`
...
export async function uploadGalleryImage(file, caption, title) {
  ...
  const { error } = await supabase.from('gallery').insert({
    title, caption, image_url: data.publicUrl
  })
  if (error) throw error
}
