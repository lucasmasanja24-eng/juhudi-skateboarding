...
export async function addMemberDirectly(memberData) {
  // Admin adds a member directly without auth account
...
export async function getTimetable() {
  const { data, error } = await supabase.from('timetable').select('*').order('sort_order', { ascending: true })
  if (error) throw error
  return data
}
